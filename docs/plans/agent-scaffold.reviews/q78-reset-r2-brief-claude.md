# Q-78 reset shared review brief, Claude lens

## Role

Review the repaired Q-78 design independently from the planner and the other reviewer.

Use a fix-verification, adoption and executability lens.

Verify what closes, what diverges and what a future implementer must do.
...
Reproduce D1, D2, D3 and D5 before the repair.

Verify each required correction against the repaired text.

For every divergence from the round 1 correction, decide whether it is sound.

Check executability from a future implementation worktree.

Check these adoption boundaries:

- Existing projects can express paragraphs without source-format restrictions.
- The optional migration phase remains parseable.
- The final required-field phase rejects absence and emptiness only.
- Human and JSON output contracts are unambiguous.
- Exact labels, indentation and blank lines agree across prose and criteria.
- Render fixture work can implement the specified heading placement.
- `next` human formatting does not corrupt unrelated context slots.
- `status --step --json` composes with the current CLI `--json` contract.
- Unknown, absent and present intent states remain distinguishable.
- Multiline values survive every shell script in the batch criteria.
- Earliest-commit searches and substring checks work for newline values.
- No command substitution silently removes meaningful trailing newlines.
- The rendered reconciliation reaches question, front and tail sidecars correctly.
- The pack and changelog impact remains scheduled in increment 3.
- The changed-path sets permit every promised edit and forbid unrelated edits.
- The exploration and registered Q-78 item reflect the human decision.
- The intake and decision records match the presented options and choice.

Check that no live design clause restores the superseded one-sentence or single-line design.

Check that every cited figure and selector reproduces on the current tree.
...
`/tmp/claude-1000/-home-jessea-Documents-projects-agent-scaffold/2fed83bd-4a13-402b-9e76-143356c0d130/scratchpad/q78-reset-r2-claude`.
