# Q-78 post-escalation round 1 Claude review brief
...
Use adoption, executability and fix verification. Assume that an implementer follows the specification literally.

Check:

- Every current-tree command and in-increment figure.
...
```text
/tmp/claude-1000/-home-jessea-Documents-projects-agent-scaffold/2fed83bd-4a13-402b-9e76-143356c0d130/scratchpad/q78-foreclosure-r1-claude
```

Create a reviewer-owned child directory. Never use bare `/tmp` or wildcard deletion. Run project commands through the project direnv environment. Use GNU grep for GNU-regex claims.
...
```text
docs/plans/agent-scaffold.reviews/q78-foreclosure-r1-reviewer-claude.md
```
