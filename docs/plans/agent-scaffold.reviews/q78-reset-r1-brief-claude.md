# Q-78 reset round 1 Claude review brief

## Role

Review the five Q-78 sidecars independently through fix verification, adoption, and executability.

Assume that the repaired specification still contains defects.

Do not read the GPT lens brief or any round 1 findings file.
...
Rebuild every round 4 demonstration against the repaired text.

Verify that each mandatory repair closes its finding.

Verify that each accepted residual stays inside its recorded boundary.

Test the commands from the working directory that the sidecar requires.

Check every changed-path set against all files a correct implementation must edit.

Check every generated copy, golden, migration record, and documentation duty.

Check dependency order and blocking edges against every statement about which increment opens a shared section.

Check that a fresh adopter can follow each criterion without inventing data or commands.

Check that the criteria neither reward fabrication nor refuse a correct implementation.
...
Verify repairs for `GB-1`, `GB-2`, `GB-3`, `GB-5`, `GB-6`, `GB-7`, `GB-8`, `GB-10`, `GB-11`, `F1`, and `T-1`.

Verify each accepted residual's severity, cost, owner, and non-expansion boundary.
...
`/tmp/claude-1000/-home-jessea-Documents-projects-agent-scaffold/2fed83bd-4a13-402b-9e76-143356c0d130/scratchpad/q78-reset-r1-claude`.
