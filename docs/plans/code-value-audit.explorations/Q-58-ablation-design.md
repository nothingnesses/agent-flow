...
The protocol commit id is recorded in the result. Once any trial output has been observed, changing one of these fields invalidates that run: the result must say so and restart under a new pre-run protocol rather than silently revising the rule. Marginal/per-arm confidence intervals are descriptive only, and neither they nor their overlap is ever a decision rule or evidence of equivalence/non-inferiority. The protocol may instead precommit an appropriate paired confidence-bound procedure or paired hypothesis test to implement the frozen non-inferiority/equivalence rule; that paired bound/test, its margin and every choice above must be fixed before results exist.
...
- Falsifiable-deletion reading of the plot: the per-state delta (V0 score minus reduced-variant score) is the value the deleted content earned. Interpret that paired delta through the frozen paired confidence bound or hypothesis test. Marginal/per-arm intervals and their overlap remain descriptive and never classify a carrier. Expect the interesting signal in the mid-round Tier-B states: if V1 drops below V0 there but V2 recovers it, the live anchor earns its keep; if V3 clears the frozen rule at lower cost, structured-only reaches the plateau.
...
- Agent variance. Consuming agents are stochastic; a single trial per cell is noise. Mitigations: N replicates and descriptive marginal/per-arm confidence intervals for uncertainty reporting. Those intervals, their overlap and point estimates never decide the knee. The frozen non-inferiority/equivalence rule, precommitted margin and model-disagreement rule decide it; when the protocol chooses a paired confidence-bound method, that bound is the decision procedure rather than a descriptive per-arm interval. Report PER MODEL, because a verbose-tolerant model may score V0 and V1 equally (masking a real cost) while a context-limited model may score V1 ABOVE V0 (the echo actively buries the signal); averaging those two hides the effect the ablation is meant to find.
