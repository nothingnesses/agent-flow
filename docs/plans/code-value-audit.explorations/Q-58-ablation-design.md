...
- Agent variance. Consuming agents are stochastic; a single trial per cell is noise. Mitigations: N replicates and confidence intervals that report uncertainty only. The knee is decided by the frozen non-inferiority or equivalence procedure, its precommitted margin and its model-disagreement rule, never by confidence-interval overlap or point estimates. Report PER MODEL, because a verbose-tolerant model may score V0 and V1 equally (masking a real cost) while a context-limited model may score V1 ABOVE V0 (the echo actively buries the signal); averaging those two hides the effect the ablation is meant to find.
