...
The design pass specified by this brief is complete. Its proof-before-choice artefact then reached the five-round risky review cap with valid findings in every round. Q-88 records the human decision to end that capped artefact and narrow the proof scope. Q-86 remains `open` against the new narrowed decision artefact at `docs/plans/workflow-calibration.explorations/Q-86-synthesis.md`.

Q-85 decided only where this investigation belongs by adding the design pass to the existing `workflow-calibration` step. Q-88 decides only proof timing. Neither decision selects a convergence mechanism or a serious terminal floor. The synthesis retains A, B, and C with their proposed algebraic finite bounds. A later human decision selects an architecture, after which a complete executable proof of concept for only that selected architecture is a blocking gate before implementation.

This pass changes no current workflow rule or implementation. In particular, it does not change the five-round per-artefact cap, the reset-after-human-resume branch, the single-pass acceptance rule, any convergence constant, any Roadmap status, or any existing review identity. The retained controller and replay scripts are adversarial prototypes that exposed proof obligations. They are not recommendation-eligibility proofs.
...
After the human selects A, B, or C, the selected architecture alone must receive a complete executable proof of concept before implementation. That proof must soundly model arbitrary finite finding maps, cross-family serious carry, scope and dismissal products, global family identity, fresh evidence, legacy adoption, both terminal floors, and every selected-option transition and bound. It must close every applicable valid verdict in `docs/plans/agent-scaffold.reviews/q86-synthesis-r1-triage.md` through `q86-synthesis-r5-triage.md` with a traceable executable assertion or a cited architecture-specific inapplicability reason. Do not mutate the live log, plan statuses, current review identities, workflow spec, pack, or production code while investigating.
