...
The design pass specified by this brief is complete. Its proof-before-choice artefact then reached the five-round risky review cap with valid findings in every round. Q-88 records the human decision to end that capped artefact and narrow the proof scope. The narrowed decision artefact is closed.

Q-86 is now `decided -> folded into bounded-convergence-option-b-proof`. The recommendation and human choice were both `B - Frozen obligations with sealed phase campaigns`. Q-91 separately records recommendation and chosen terminal floor `high` from the exact options `high`, `critical`, and `defer`. The synthesis retains A, B, and C with their proposed algebraic finite bounds as decision provenance. The selected Option B proof is a complete executable blocking gate before implementation.

This decision fold changes no current workflow rule or implementation. In particular, it does not change the five-round per-artefact cap, the reset-after-human-resume branch, the single-pass acceptance rule, any convergence constant, or any existing review identity. The new Roadmap unit is planning and proof work only. The retained controller and replay scripts are adversarial prototypes that exposed proof obligations. They are not recommendation-eligibility proofs.
...
3. A provisional recommendation, selected only from the viable bounded mechanisms, with reasoning, confidence, and the evidence that would overturn it. The recommendation must rest on the architecture and evidence rather than on the prototype controller's zero counters. It is advice for the later human choice and does not make a mechanism implementation-eligible.
...
5. Falsifiable stopping properties, proposed algebraic finite bounds, and a red-control inventory for each viable bounded mechanism. At minimum, cover repeated human resume, an unbounded acceptance-repair sequence, a fix-induced high finding near exhaustion, scope expansion, relitigation without new evidence, narrowing or replan, and an unresolved critical at exhaustion. Before the architecture choice, prototypes may expose missing states and compare bounds without claiming complete proof. After the human selects an architecture, its complete executable proof of concept must establish every applicable path and bound before implementation. For the non-admissible baseline, demonstrate and report the repeated-resume and later-acceptance counterexamples that prevent a finite bound.
...
Before the human architecture choice, candidate scripts are adversarial prototypes. They may demonstrate a counterexample, expose a proof obligation, or support an algebraic comparison. They must not be presented as complete stopping or unresolved-critical proofs.

Selected Option B alone now receives a complete executable proof of concept before implementation. That proof must soundly model sealed plan review, finite frozen obligations and exact phase ownership, arbitrary finite finding maps, cross-owner and mixed products, cross-family serious carry, scope and dismissal products, global family identity, fresh evidence, legacy adoption, both terminal floors, terminal choices, and every selected transition and bound. It must close every applicable valid verdict in `docs/plans/agent-scaffold.reviews/q86-synthesis-r1-triage.md` through `q86-synthesis-r5-triage.md` with a traceable executable assertion or a cited Option-B-specific inapplicability reason. Do not mutate the live log, plan statuses, current review identities, workflow spec, pack, or production code while proving it.

## Documentation impact

This decision fold changes only planning records, so no shipped pack or code documentation becomes stale now. The proposal inventory remains the input to a later production plan only after the proof passes and the human separately authorises implementation. Implementation remains blocked by `bounded-convergence-option-b-proof`.
