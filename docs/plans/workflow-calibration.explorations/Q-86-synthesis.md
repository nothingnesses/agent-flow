...
The decision fold materialises `bounded-convergence-option-b-proof` as the complete executable proof of concept for selected Option B. Its `complete` status, not `skipped` or any other non-complete status, is an explicit blocker for every later production implementation unit for bounded convergence. Such a future unit must use the typed `[[step.blocker]] target = "bounded-convergence-option-b-proof"` and `policy = "complete-only"` contract planned in `workflow-ready-frontier-scheduler`; the generic complete-or-skipped `blocked_by` edge is insufficient for this safety gate. No production unit is added here. The proof of concept is a blocking specification test, not production controller code, and it must converge through the ordinary risky review loop.
...
Only after the selected proof of concept passes may the implementation plan schedule changes to `.agents/workflow.toml`, `pack/workflow.toml`, `WorkflowSpec`, shared `ReviewProcess` reconstruction, metrics, plan state, ledger projection, `validate --workflow`, `next`, pack guidance, prompts, templates, generated copies, README, and changelog. Every such production unit must carry the typed complete-only proof blocker; the scheduler and validator must keep it blocked for `skipped` and every other non-`complete` proof state while preserving complete-or-skipped semantics for ordinary blockers. Historical records remain append-only and receive explicit legacy treatment rather than inferred structure.
