...
Blind closure is an explicit discovery stage, not content-free completion credit. A valid current-owner finding or overturned serious dismissal cannot repair and then deliver on stale closure evidence; it terminalises delivery or requires a receipted replan. Completed-prior and unowned closure routes remain non-delivering. A valid future-owner component remains `FutureOwnerPending` while the discovering and intervening phases complete, activates only in its canonical later campaign, and that campaign supplies its own closure after settlement. A serious dismissal leaves closure pending until an independent re-check; upheld settles that identity, while overturned follows its owner route. In particular, the selected-option proof must show arbitrary finite batch handling, exact owner-state and phase-route composition, those blind-closure fixtures and killing mutations, fresh-evidence reopen, globally unique successor identity, serious carry into successors, legacy adoption, every floor outcome, and terminality without relying on the prototype's global finding cap.
...
3. Model atomic cross-owner batches, mixed owned and unowned findings, low plus critical, parent plus fix-induced child, valid plus serious-dismissal, scope plus dismissal, and multiple simultaneous scope re-checks without scalar erasure or bulk rewriting unrelated identities. Distinguish discovery phase from canonical owner phase; prove current-, completed-prior-, and future-owner routes with acceptance-to-completed-work, early-work-to-future-owner, and mixed out-of-phase fixtures and killing mutations. Model blind closure as an explicit discovery stage with current-, completed-prior-, future-, and unowned routes plus upheld and overturned serious dismissals; kill stale-evidence delivery after a current-owner closure finding, non-delivering prior/unowned escapes, future-component terminalisation or loss, premature family completion, and completion without settled closure evidence. Retain every cross-owner finding while awarding initial-attempt credit only to the scheduled owner.
