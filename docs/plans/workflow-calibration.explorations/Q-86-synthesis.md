...
Q-86 is `decided -> folded into bounded-convergence-option-b-proof`. The exact architecture options were `A - Sealed phase budget`, `B - Frozen obligations with sealed phase campaigns`, and `C - Fixed-depth verification protocol`. The recommendation and human choice were both Option B. Q-91 separately records the exact terminal-floor options `high`, `critical`, and `defer`, with recommendation and chosen value `high`. Q-92 later records only the capped proof-plan specific-fix closure path; it changes neither selection.
...
No controller constant or production implementation has been selected. Q-85 authorised the design pass. Q-88 changed only the proof timing and gate. Q-89 retired the capped narrowed counters and authorised only the two low repairs plus one focused mixed-model verification. Q-90 authorised only the exact current-state wording correction and merge without another review. Q-86 selects Option B and Q-91 selects floor `high`, but both authorise proof work only. After their proof-plan fold reached its five-round cap, Q-92 authorised only the four adjudicated repairs plus one focused GPT-and-Claude verification; those capped planning counters retire, while the future proof's unwaived risky round bar remains unchanged.
...
For post-freeze phase `q`, let `n_q = |O_q|`. Each obligation has at most an initial attempt and verification, one materially-new-evidence reopen attempt and verification, while the phase has one non-transferable blind-closure discovery batch. An atomic batch may retain findings for several owners, but only its one scheduled owner receives initial-attempt credit; every other untested owner still requires its own scheduled batch. The proposed per-phase maximum and clean, ordinarily-delivering `Complete`-domain minimum are:
...
With `p = m + 1` post-freeze phases, exact ownership and no transfer give the proposed whole-family maximum and clean, ordinarily-delivering `Complete`-domain minimum:
...
These are algebraic design bounds. The controller prototype does not prove their safety premises. `L_q` ranges only over phase segments of clean, ordinarily delivering family paths that have a converged sealed review, valid exact freeze, a scheduled phase whose own obligations and blind-closure discovery settle without an intervening terminal or non-delivery transition, and eventual family `Complete`. `L_B` ranges only over clean, ordinarily delivering family paths with valid sealed review and freeze, every post-freeze phase and delivery predicate settled, and final `Complete`. Domain membership does not assume that every owner received a separate scheduled batch; the selected transition relation must derive that fact, and therefore `L_q = n_q + 1`, from scheduled-owner-only credit. Legal early-terminal and non-delivery paths may be shorter and lie outside both minima.

Blind closure is an explicit discovery stage, not content-free completion credit. Its component state is the full `OwnerRoute x ScopeDisposition x TriageDisposition` product under one stable finding id; resolving scope, dismissal, or owner routing cannot erase another axis. Low scope expansion becomes backlog, medium takes the terminal human scope decision, high keeps closure pending through scope re-check then goes to that terminal decision when upheld or returns in scope and takes its applicable owner route when overturned, and critical becomes `SeriousBlocked` on either scope result. Closure remains pending while either a scope or serious-dismissal re-check is unresolved, including legal atomic products in which both are pending. An overturned dismissal likewise follows the scope result before owner routing, while an upheld dismissal settles only its own axis.

A valid current-owner finding or an in-scope overturned serious dismissal cannot repair and then deliver on stale closure evidence; it terminalises delivery or requires a receipted replan. Completed-prior and unowned closure routes remain non-delivering. A valid future-owner component remains `FutureOwnerPending` while the discovering and intervening phases complete, activates only in its canonical later campaign, and that campaign supplies its own closure after settlement. In particular, the selected-option proof must show arbitrary finite batch handling, exact scope, dismissal, owner-state, and phase-route composition, low/medium/high/critical closure fixtures, atomic scope-plus-dismissal products, per-axis re-check reducers, and killing mutations for pending-re-check bypass, overturned-high owner-route loss, and critical escape from `SeriousBlocked`, as well as fresh-evidence reopen, globally unique successor identity, serious carry into successors, legacy adoption, every floor outcome, and terminality without relying on the prototype's global finding cap.
...
The Option B `L_q` and `L_B` cells are minima only for their clean, ordinarily delivering `Complete` domains. Legal early-terminal and non-delivery executions lie outside those domains and may consume fewer batches.
...
The decision fold materialises `bounded-convergence-option-b-proof` as the complete executable proof of concept for selected Option B. Future production readiness requires the proof's exact `complete` status, direct round-backed convergence at its declared risky two-clean-round bar, and no step or increment convergence waiver of either evidence tier. Such a future unit must use the typed `[[step.blocker]] target = "bounded-convergence-option-b-proof"` and `policy = "complete-only-unwaived"` contract planned in `workflow-ready-frontier-scheduler`; the generic complete-or-skipped `blocked_by` edge and W3's generic convergence-waiver route are insufficient for this safety gate but remain unchanged for ordinary steps. No production unit is added here. The proof of concept is a blocking specification test, not production controller code, and it must converge through the ordinary risky review loop without a convergence waiver.

The proof of concept must:

1. Define the selected architecture's complete typed state and every legal transition, including plan review, work review, acceptance, repair, verification, re-check, exhaustion, terminal decision, resume reconstruction, narrowing, replan, revert, abandonment, and completion where the option permits them. Replay one identical ordered post-freeze event stream through uninterrupted baseline, resume, rename, rebuild, and unchanged replan and compare architecture, floor, family id, predecessor and current spend, remaining authority, complete finding map, owner routes, and terminal state without replenishment.
...
3. Model atomic cross-owner batches, mixed owned and unowned findings, low plus critical, parent plus fix-induced child, valid plus serious-dismissal, scope plus dismissal, and multiple simultaneous scope re-checks without scalar erasure or bulk rewriting unrelated identities. Distinguish discovery phase from canonical owner phase; prove current-, completed-prior-, and future-owner routes with acceptance-to-completed-work, early-work-to-future-owner, and mixed out-of-phase fixtures and killing mutations. Model blind closure as the full owner, scope, and dismissal product: low backlog; medium terminal scope decision; high pending/upheld/overturned scope re-check, with an overturned high then taking its applicable owner route; critical `SeriousBlocked` under either scope result; and closure pending while either a scope or dismissal re-check remains. Include atomic scope-plus-dismissal fixtures and per-id, per-axis reducers. Kill stale-evidence delivery after a current-owner closure finding, non-delivering prior/unowned escapes, future-component terminalisation or loss, pending-re-check bypass, one re-check clearing the other, overturned-high owner-route loss, critical escape from `SeriousBlocked`, premature family completion, and completion without settled closure evidence. Retain every cross-owner finding while awarding initial-attempt credit only to the scheduled owner.
...
9. Prove every selected-option transition and algebraic bound, including reserve and clean-suffix foreclosure for A, the inherited plan controller, obligation ownership, scheduled-owner-only initial credit as a derived transition consequence, phase routing and blind campaigns for B, or fixed stages and mandatory acceptance blind closure for C. Define `L_q` and `L_B` only over independently observable clean, ordinarily delivering `Complete` conditions; keep the two-owner cross-credit mutant in that domain and require it to make `L_q < n_q + 1`.
10. Carry a traceability matrix for every valid verdict in `docs/plans/agent-scaffold.reviews/q86-synthesis-r1-triage.md` through `q86-synthesis-r5-triage.md`. Every valid finding must be marked applicable and closed by a named executable assertion, or inapplicable with a cited architecture-specific reason. Every executable row's named mutation must resolve to exactly one mutation-manifest entry whose result is exercised and killed; unknown, surviving, or unexercised names fail. Planning and documentation findings must point to their corrected durable text rather than being silently omitted.
11. Include mutation controls that remove each load-bearing transition or predicate and demonstrate that the proof fails, including drift or replenishment of each compared post-freeze field under resume, rename, rebuild, and unchanged replan.
...
Only after the selected proof of concept passes may the implementation plan schedule changes to `.agents/workflow.toml`, `pack/workflow.toml`, `WorkflowSpec`, shared `ReviewProcess` reconstruction, metrics, plan state, ledger projection, `validate --workflow`, `next`, pack guidance, prompts, templates, generated copies, README, and changelog. Every such production unit must carry the typed `complete-only-unwaived` proof blocker; the scheduler and validator must keep it blocked for `skipped`, every other non-`complete` proof state, roundless or below-streak `complete`, and self-declared or record-backed step- or increment-waiver completion while preserving complete-or-skipped and convergence-or-waiver semantics for ordinary steps. Historical records remain append-only and receive explicit legacy treatment rather than inferred structure.
...
Q-86 records the exact architecture options `A - Sealed phase budget`, `B - Frozen obligations with sealed phase campaigns`, and `C - Fixed-depth verification protocol`, with recommendation and chosen value Option B. Q-91 records the exact floor options `high`, `critical`, and `defer`, with recommendation and chosen value `high`. Q-92 records the five cap choices in order and the selected `Fix, verify once, then close` path for the proof-plan artefact.

All three receipts use `task:"bounded-convergence-option-b-proof"`. Q-86 and Q-91 authorise only that proof-of-concept unit; Q-92 authorises only the four planning corrections and one focused verification. None authorises production implementation.
