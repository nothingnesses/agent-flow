...
Explorer proposal for `Q-86`, written under a safety, human-process, finding-lineage and reviewer-allocation lens. Advisory design notes only. This document changes no workflow rule, no constant, no status and no code. Production implementation remains blocked on `bounded-convergence-option-b-proof` reaching `complete`, not `skipped` or any other non-complete state, and on a later separate implementation decision. The future typed complete-only dependency and its scheduler/validation enforcement are planned in `workflow-ready-frontier-scheduler`; no production unit is added here.
...
Q-85 decided only where this investigation belongs. Q-86 and Q-91 authorise only `bounded-convergence-option-b-proof`. Its gate is complete-only: a future production unit must use the typed blocker policy planned in `workflow-ready-frontier-scheduler`, whose red controls keep that unit blocked when the proof is `skipped` or in any other non-`complete` status while ordinary blockers retain complete-or-skipped semantics. No production implementation is approved and no production unit exists.
