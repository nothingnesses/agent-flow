...
When every obligation and pre-closure current-owner component in one phase is settled, its non-transferable `BlindClosureDiscovery(phase_id)` stage runs one atomic triaged discovery batch. Closure is not awarded merely because the batch ran. The complete result has these routes:

- A valid current-owner finding, or an overturned high-or-critical dismissal for that owner, becomes `ClosureCurrentOwnerBlock`. The only closure batch cannot serve as evidence about a later repair, so no automatic post-closure repair can lead to delivery; only terminal non-delivery or receipted replan remains.
- A valid or overturned completed-prior-owner finding remains live under the immutable prior owner and stays non-delivering, with only terminal non-delivery or receipted replan.
- A valid or overturned future-owner finding becomes `FutureOwnerPending`. The discovering phase may complete without spending or activating it; it remains live through intervening phases, activates only in its canonical campaign, and that later campaign supplies its own blind-closure discovery after the component settles.
- A valid or overturned unowned finding remains `UnownedInScopeFinding` and non-delivering; an unowned critical enters `SeriousBlocked`.
- Any high-or-critical dismissal keeps closure pending in `AwaitingDismissalRecheck`. An independent upheld result settles that identity and permits closure to settle when no other component blocks it. An overturned result follows the applicable owner route above.
