...
- FR-FLEET-1 `[CONTROL]` S2. Model the workflow as a FLEET of per-unit FSM instances (round 1's hierarchical model), not one global machine: one task machine, one step machine per Roadmap step (statuses reuse the existing `StepStatus` enum), and one disjoint `ReviewProcess` per active review. `Convergence(ReviewLoop)` covers exact task/Roadmap plan review and Roadmap work review; `SinglePass(SinglePassReview)` covers acceptance/standalone review. Each process carries its phase-bearing id and awaiting state, but only `Convergence` can carry class/streak/foreclosure/cap state. (P5, P1.)
...
Historical staging note: at S1 (advisory MVP) the tool computed and reported one unit's state at a time, which was the already-decided `state-queries` scope. Q-82 later superseded the real-parallelism precondition and scheduled the typed fleet before the read-only scheduler because phase correctness and plural visibility supplied independent evidence. This supersession covers only the typed fleet/read-only scheduler; it grants no write path, fan-out, or authoritative-driving permission, whose separate gates remain.
...
- FR-LOOP-5 `[JUDGMENT-IN]` S1. CONSUME, never produce: the triager's round/pass result, authoritative declared `risk_class` for a convergence loop, backstop `recheck_result`, and human decision at an escalation. The tool blocks awaiting these; it never invents a verdict or derives class from a round. (AGENTS.md WP-18, WP-5; this is the no-judgment invariant applied.)
- FR-LOOP-6 `[CONTROL]` S1/S2. Model convergence and single-pass processing as disjoint variants, never as one shared loop. `Convergence` alone has declaration/class, round/streak, foreclosure, cap, convergence, and escalation; `SinglePass` has none of those constructible states. Both gate a high/critical dismissal on the same backstop threshold, and an overturned single-pass re-check routes a valid finding/shortfall without constructing a streak. (P5, P8.)
...
Historical contested point (is the scheduler in scope at all): the skeptic argued that a solo/small-team dogfood did not justify fleet/scheduler machinery. Q-82 later supplied phase/visibility evidence and chose staged delivery, so the real-parallelism precondition is superseded for the typed fleet and read-only ready-frontier scheduler. The focused scheduler still only computes/proposes and never fans out, spawns, or writes. The guarded write path remains gated on measured advisory adoption plus reopening Q-24, and authoritative driving remains gated on measured-low override rate; Q-82 does not supersede either.
...
- The per-unit FSM fleet and the `blocked_by` ready-frontier scheduler (FR-FLEET, FR-SCHED). (Retained S2 label. Q-82 schedules the typed fleet and supersedes the former real-parallelism gate for the typed fleet and read-only scheduler only; `docs/plans/agent-scaffold.steps/workflow-ready-frontier-scheduler.md` is the current Stage-3 authority. This bounded supersession does not lift the separate advisory-adoption/Q-24 gate on the write path or the measured-override gate on authoritative driving.)
...
- Authoritative-blocking driving (the tool refuses to advance without the recorded event). (S3, gated on a measured-low override rate.)
...
- Computing any judgment: no auto-triage, no auto/default risk-classification, no auto "small and reviewable", no auto ceremony-collapse, no auto acceptance, no trivial/non-trivial decider. (NFR-NOJUDGE.)
...
The recommended scope line for "the full driver" was therefore: everything in section 13's IN list is the TARGET, but the build order is S1 (advisory MVP on the spec, the natural `state-queries` increment) -> S2 (fleet + scheduler + guarded write-path, then described as gated on real parallelism and advisory-adoption evidence, and on reopening Q-24) -> S3 (fragment generation + authoritative-blocking, gated on the generation proof and measured override rate). Q-82 is a bounded later supersession of that sentence: it schedules the typed fleet and lifts the real-parallelism gate for the typed fleet/read-only scheduler only. The guarded write path still waits on advisory-adoption evidence and Q-24 reopening, and authoritative driving still waits on its measured-override gate. The scope remains the full driver; the surviving gates keep write/authority stages honest against P6.
...
- SUPERSEDED BY Q-82 for the typed fleet and read-only scheduler: do not use the absence of real parallel execution to block those scheduled focused steps. The scheduler still only proposes a ready frontier; do NOT add fan-out/spawning here, do NOT build the `record-*` write path before measured advisory adoption plus Q-24 reopening, and do NOT flip to authoritative driving before override rate is measured low.
...
- Do NOT compute any judgment, ever (NFR-NOJUDGE): no auto-triage, no auto/default risk class, no auto acceptance, no trivial/non-trivial decider.
