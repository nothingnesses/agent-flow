...
- Stage 3+ (partly scheduled): Q-82 lifts the former real-parallelism gate for the typed fleet and read-only `blocked_by` scheduler only and makes `docs/plans/agent-scaffold.steps/workflow-ready-frontier-scheduler.md` the scheduler authority; that sidecar fixes step-only eligibility, skipped-blocker policy, and explicit-action precedence. Generation-widening beyond constants, the `record-*` write path (gate: advisory adoption evidence + reopening Q-24's no-write-path stance), authoritative/blocking driving (gate: measured-low override rate), and the far corner (calibration closure, workflow-viz as a read, multi-repo) remain gated.
