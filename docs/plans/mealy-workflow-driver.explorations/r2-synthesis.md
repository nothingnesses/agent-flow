...
- Stage 2 (RISKY, focused successor dependencies): type the FSM engine in `src/driver/` as disjoint `ReviewProcess::{Convergence(ReviewLoop), SinglePass(SinglePassReview)}` variants, nested task/question/step machines, and a stateless `reconstruct` layer consuming the shared phase-bearing declaration/selection/process reconstruction so driver and checker provably run the same authority joins and arithmetic. Single pass retains high/critical dismissal re-check but cannot construct class, streak, foreclosure, or cap.
- Stage 3+ (partly scheduled): Q-82 lifts the former real-parallelism gate for the typed fleet and read-only dependency scheduler only and makes `docs/plans/agent-scaffold.steps/workflow-ready-frontier-scheduler.md` the scheduler authority; that sidecar fixes step-only eligibility, ordinary complete-or-skipped blockers, typed unwaived complete-only proof blockers that require round-backed convergence, and explicit-action precedence. Generation-widening beyond constants, the `record-*` write path (gate: advisory adoption evidence + reopening Q-24's no-write-path stance), authoritative/blocking driving (gate: measured-low override rate), and the far corner (calibration closure, workflow-viz as a read, multi-repo) remain gated.
...
A bootstrap "planning complete" signal; a `record-step-start` event (per-unit diff-base, so the ledger becomes purely human narrative and supports parallel in-flight units); a loop-reset marker after a send-back so total rounds are not double-counted; extending W5 to accept `record-override` as completion evidence (parallel to waivers); and pre-computing findings-file paths in `next` output. The earlier candidate to default an omitted class to `risky` is superseded by Q-83 and its acceptance repairs: live Roadmap plan/work review resolves exact increment declarations, task plan review resolves a task declaration, Q-81 Roadmap-increment and exact-step plan review are bounded non-active history, phase stays in round/escalation/active identity, and the exact active row selects before round one. Missing declarations/selections and future unowned work review fail closed; bounded malformed history remains typed; each convergence round carries a matching auditable snapshot; and acceptance/standalone review use a disjoint recheck-capable single pass rather than inheriting a bar.
