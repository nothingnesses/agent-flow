...
- Stage 2 (RISKY, focused successor dependencies): type the FSM engine in `src/driver/` (a spec-parameterized `ReviewLoop` Mealy machine, nested task/question/step machines, and a stateless `reconstruct` layer consuming the shared declaration/selection and loop-window reconstruction so the driver and checker provably run the same authority joins and arithmetic).
