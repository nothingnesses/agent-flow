...
- Disjoint review-process machines: `Convergence(ReviewLoop)` owns exact phase-bearing plan/work review, declared class, streak, cap, precedence, resets, and the backstop gate; `SinglePass(SinglePassReview)` owns acceptance/standalone review with the same high/critical re-check but no constructible class, streak, foreclosure, or cap. Round 1's `fsm-concurrency-model.md` and the concrete architecture specify the total typed transitions; convergence arithmetic is the checker relation run forward.
...
- P6 (evidence-first) and W3/W6 (ground in evidence, verify): the staging exists to serve P6. Q-82 later supplied phase/visibility evidence and superseded the real-parallelism gate for the typed fleet and read-only scheduler only. Other gates remain evidence-specific: adoption/persistent drift for write-path work, generation readability for widening, low override rate for authoritative driving, and enough data for calibration. This is the bounded current reading of the earlier MVP-first argument.
...
- P2 (minimal by default): the MVP is minimal and the far corner maximal. P2 is the tension the gates manage. Q-82 has now scheduled the typed fleet/read-only scheduler on correctness and visibility evidence rather than parallel scale; calibration remains behind its data-volume gate, the write path behind advisory adoption plus Q-24 reopening, and authoritative driving behind measured-low override rate.
...
Net: evidence, minimalism, and stability still require staged work, while clean structure and one source support the full north star. Q-82 changed one premise: real parallelism is no longer a precondition for the typed fleet/read-only scheduler because phase correctness and plural visibility independently earned them. It did not authorise fan-out, writes, or authoritative driving, so those surviving gates preserve the original principle balance.
...
- LATER STAGES (current supersession): Q-82 has scheduled the typed fleet and read-only scheduler in focused steps on phase/visibility evidence. Event-recording still waits on measured advisory adoption plus Q-24 reopening; authoritative blocking waits on measured-low override rate; generation widening waits on the proven fragment; calibration/viz/multi-repo retain their own gates. This bounded update preserves the staged recommendation without treating its former parallel-scale premise as live.
...
- The tool making any JUDGMENT: auto-triage, auto/default risk-classification, auto "small and reviewable," auto ceremony-collapse, auto-acceptance. Determinism ends at the verdict; the tool consumes verdicts, never computes them (W18). This boundary is permanent.
