...
THE INPUT IS A PROPERTY, NOT A FROZEN SLUG COUNT. At this step's base, rerun the predecessor's anchored selector over the generated Roadmap and capture its complete output. Because the predecessor has removed its own worklist, the remaining rows are this step's handover. Record the list in the outcome before editing. The 2026-08-21 decision was taken against the then-current measured set; Q-84 later chose to continue when the anchored handover remeasured at 18 after `workflow-driver` left it. Both decisions preserve the selector at the implementation base as authority, so no implementation criterion hard-codes either dated count.
