...
10. THE CHANGED PATH SET. `git diff --name-only` lists exactly `CHANGELOG.md`, `src/main.rs` and `tests/validate_refuses_a_missing_explicit_path.rs`, the file criterion 8 adds. No file under `docs/plans/` appears, and `pack/` does not appear, because the pack ships no `validate` invocation that names a path this rule newly rejects. If that measured pack exclusion turns out to be false at implementation time, the relevant pack file joins the set and the outcome says which stale promise required it; the exact set must never force a correct documentation update out of the increment.
...
`CHANGELOG.md` records this shipped behaviour change under `## [Unreleased]`, creating that section if no earlier pending step has done so. The entry states the boundary, not only the symptom: an explicitly supplied missing `--source`, `--plan` or `--metrics` path now produces the named error and exit 1, while an absent default-derived metrics path still notes and skips. It also gives the upgrade action for a project whose `.agents/checks.toml` names a path that moved: correct the path or remove the explicit flag. This is the same externally visible false-green-to-failure class the existing 0.0.2 `principles.toml` entry documents, and criterion 10 admits the changelog path so an implementation cannot be forced to choose between behaviour correctness and documentation currency.

`README.md` IS NOT MADE STALE, MEASURED. Its `validate` section documents source/metrics path resolution and the workflow pairing, but it does not promise that a missing explicit path notes and exits 0. The changed exit behaviour therefore falsifies no README sentence, and the README remains outside criterion 10's exact path set. Re-run that reading at implementation time; if the README acquires such a promise first, it joins the path set under the same fail-loud rule.
