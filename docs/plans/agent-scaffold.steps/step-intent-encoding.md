...
THE EMPTY-BODY, FIELD-BEARING BRANCH HAS ITS OWN UNIT ORACLE. A test in `src/plan/render.rs`, named `intent_only_step_emits_complete_step_details_fragment`, constructs one step with `problem = Some("empty-body problem")`, `approach = Some("empty-body approach")` and an empty Markdown body, then calls `step_details_section`. It asserts that the complete returned `Some` value, with no ignored prefix or suffix, is byte-for-byte:
...
The final byte is the final `h` of `approach`; the expected value has no trailing newline. Temporarily restore the old `body is non-empty` contribution guard and record this test failing because the function returns `None`; remove the guard and run the test green. Both fields are present so the exact test remains representable after increment 3 makes them required. This is the increment-1 oracle for the rule that intent contributes a Step Details entry even when the body contributes no bytes.

THIS CRITERION IS THE ONE THAT CATCHES THE `title` TRAP. Against an implementation that adds both fields to the schema and emits neither, the schema and query criteria can pass while the census prints zero. Criteria 4 to 6 pin field ownership, placement, spacing, paragraph preservation, the empty-body branch and the partial case inside the labelled blocks.
...
THEN REPEAT BOTH COMPLETE MATRIX PASSES IN THE NON-REVIEW WRITER STATE `awaiting-fixes`. Reach it with the same one-step `in-progress` source plus exactly one joined round record whose outcome is `new_valid`; fix its three non-intent context values to `worktree`, `ledger.md` and `reviews/triage.md`. For every criterion 2 fixture, the JSON test compares both intent strings byte-for-byte with their independent parser oracles AND asserts that the complete context has exactly five keys and these three other values. On the single-paragraph row the complete JSON context is:

```
{
  "approach": "single approach paragraph",
  "isolation_tier": "worktree",
  "ledger": "ledger.md",
  "problem": "single problem paragraph",
  "triage_findings": "reviews/triage.md"
}
```

For every matrix row, no `review_findings` key exists. The human test compares the complete five-key range for every fixture with the same independent display formatter used above. On the single-paragraph row it is exactly:
...
THE LOOP-STATE RED CONTROL IS REQUIRED AND REPORTS THE WHOLE OMITTED SET. Temporarily replace the unconditional intent insertion with insertion only for the states the projection checks already sample: `ReadyToPlan`, `AwaitingFirstReview`, `AwaitingReviewers` and `AwaitingFixes`. The seam test traverses every row before asserting, accumulating a state-labelled mismatch for each absent key rather than stopping at the first failure. Record the test failing for BOTH `problem` and `approach` on EACH of `Blocked`, `Converged`, `Escalate`, `RiskClassConflict` and `Done`, while the four sampled rows and both existing per-surface state matrices stay green. Restore unconditional insertion, run `build_context_carries_intent_in_every_loop_state` green over all nine rows, then run both complete state matrices green.

8. `status --step` ANSWERS EVERY DECLARED DATA STATE, AND THE FULL LOGICAL-VALUE MATRIX RUNS THROUGH BOTH FIELDS ON BOTH SURFACES. Run the human command on EVERY criterion 2 fixture, cases (a) through (h), each carrying its case as both `problem` and `approach`:

```
./target/debug/agent-flow status --source <matrix.plan.toml> --step a
```

Compare each complete output byte-for-byte with the independent human formatter derived from that fixture's parser-oracle values. The matrix therefore covers the single paragraph, two paragraphs, repeated blanks, outer whitespace, CRLF, bare CR, continuation and interior-line-whitespace cases for BOTH fields on this exact surface. The repeated-blank values have two consecutive `  >` lines; the two carriage-return cases each produce separate `  > ` lines and no output contains a carriage-return byte; the continuation stays one display line; and the interior-line-whitespace values retain both leading and trailing spaces on each interior non-empty line. The outer-whitespace fixture's complete expected human output is explicitly:
...
Neither outer run appears anywhere in that block, for either field. The test `human_status_step_preserves_the_display_matrix` owns all eight complete two-field comparisons.

Then run the JSON command on EVERY criterion 2 fixture, again with each case in both fields:

```
./target/debug/agent-flow status --source <matrix.plan.toml> --step a --json
```

For every run, assert that the object fields occur in the order `step`, `found`, `problem`, `approach`, that `step` is `a` and `found` is `true`, then extract `problem` and `approach` after JSON decoding with `jq -j` to separate files and `cmp` EACH with that field's independent parser-oracle bytes. Extraction adds no newline. Thus outer whitespace, repeated blanks, CRLF, bare CR, continuation and interior whitespace remain unchanged for both fields. In particular, the interior-line-whitespace row compares `problem` with `b"problem paragraph one\n  problem indented interior line  \nproblem paragraph three"` and `approach` with the corresponding `approach` bytes, including the two spaces on both sides of each interior line. The named test `status_step_json_preserves_the_parser_value_matrix` performs all sixteen field comparisons in Rust; a CRLF-only or one-field test does not satisfy it.
...
THE STATUS MATRIX HAS TWO FIELD-AND-SURFACE-SPECIFIC RED MUTATIONS IN ADDITION TO THE SHARED-HELPER CONTROLS. First, in the human `status --step` `approach` branch only, skip whole-value trimming while leaving `problem` and every other projection unchanged. Record `human_status_step_preserves_the_display_matrix` failing the outer-whitespace `approach` comparison while its `problem` comparison and all other named projection tests stay green; restore it. Second, in the JSON `status --step` `approach` insertion only, trim every logical line before serialisation while leaving `problem` and every other projection unchanged. Record `status_step_json_preserves_the_parser_value_matrix` failing the interior-line-whitespace `approach` comparison while its `problem` comparison and all other named projection tests stay green; restore it. These controls make both field axes load-bearing on both status surfaces rather than relying on a shared helper to couple them.
...
THE HUMAN DECISION THIS INCREMENT WAITED ON IS TAKEN (2026-08-21, receipt `q_id:"Q-78-requiredfields"`). `problem` AND `approach` ARE REQUIRED, IN THE SHIPPED PACK AS WELL AS HERE, over an optional field with a validation warning. The human rejected a third option, required here and optional in the pack. The human weighed the cost this increment states below, and accepted the residual that a placeholder satisfies a required field forever. So this increment builds what it already specifies; the final Q-78 acceptance removed its design-review condition.
...
THEN RUN ALL FOUR TOML PROJECTION SURFACES AGAINST THIS FRESH SCAFFOLD, BEFORE LEAVING ITS DIRECTORY:

```
./target/debug/agent-flow next --source docs/plans/TEMPLATE.plan.toml
./target/debug/agent-flow next --source docs/plans/TEMPLATE.plan.toml --json
./target/debug/agent-flow status --source docs/plans/TEMPLATE.plan.toml --step example-step
./target/debug/agent-flow status --source docs/plans/TEMPLATE.plan.toml --step example-step --json
```

Pass for human `next`: its context carries an `approach:` slot whose quoted value is exactly `> <how this step addresses it>` and a `problem:` slot whose quoted value is exactly `> <the problem this step addresses>`. Pass for JSON `next`: after JSON decoding, `.active_loop.next_instruction.context.problem` is exactly `<the problem this step addresses>` and `.active_loop.next_instruction.context.approach` is exactly `<how this step addresses it>`. Pass for human TOML `status --step`: stdout is exactly the five lines `step: example-step`, `problem:`, `  > <the problem this step addresses>`, `approach:`, `  > <how this step addresses it>`. Pass for JSON TOML `status --step`: the fields occur in the order `step`, `found`, `problem`, `approach`; `step` is `example-step`, `found` is true, and both strings equal those same required placeholder values after JSON decoding. All four commands exit 0 and stderr is empty. These commands exercise the freshly copied required fields through both `next` surfaces and both TOML status surfaces; render/validate success alone does not satisfy the criterion.
...
Four DESIGN residuals. Each is recorded so a later review round does not file it as a fresh finding. This is their single home; the structured `Q-78` item and the design exploration point to this complete four-item design set rather than maintaining another copy or claiming it inventories the separately owned review residuals. The 2026-08-26 closing receipt accepts Q-78 as a whole and therefore covers all four.
