...
The repeated-blank, outer-whitespace, interior-line-whitespace, CRLF and bare-CR rows retain the exact display behaviour specified for the review state, with only `review_findings` absent. The same named tests, `next_json_preserves_the_parser_value_matrix` and `human_next_preserves_the_display_matrix`, own BOTH state families; a row in only one state does not satisfy either test's matrix obligation. THE LOOP-STATE RED CONTROL IS REQUIRED: temporarily move both intent insertions inside the `AwaitingFirstReview | AwaitingReviewers` match arm. Record every original review-state row staying green while both named tests fail their `awaiting-fixes` rows for absent `problem` and `approach`; restore unconditional insertion and run both complete state matrices green.
...
THE INCREMENT-1 NAMED PROJECTION CONTRACT IS THIS ONE AUTHORITATIVE LIST: `render_is_deterministic_and_matches_the_golden`, `each_intent_field_projects_once_inside_its_own_step_details`, `intent_only_step_emits_complete_step_details_fragment`, `intent_precedes_a_body_without_a_heading`, `next_json_preserves_the_parser_value_matrix`, `human_next_preserves_the_display_matrix`, `human_status_step_preserves_the_display_matrix`, `status_step_json_preserves_the_parser_value_matrix` and `status_step_preserves_both_partial_states`. Each name must resolve to exactly one test, every test passes after all red controls are restored, and increment 3 retains this same list and these expectations rather than defining a weaker replacement contract.
