...
THIS INCREMENT WAITS ON A HUMAN DECISION THAT IS STILL OWED. Whether `problem` and `approach` STAY REQUIRED is the human's judgement about the product's users. The design pass states the cost and makes no recommendation, and no human has weighed it. Do not start this increment until the human answers. Increments 1 and 2 stand whichever way it goes, because optional fields plus a full backfill deliver the query and the record either way.
