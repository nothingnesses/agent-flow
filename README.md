...
`next` uses `.agents/work.toml` by default when that file exists; the same source can be named explicitly with `agent-flow next --source .agents/work.toml`. The typed version-1 file contains ordered `[[step]]` entries and one `selected_action` id. `next` rejects malformed TOML, duplicate step ids, a missing selected id, or a selected step whose status is not `active`. Human and JSON output list every active unit in file order and one selected action with its user problem, change, acceptance criteria, and why-next rationale. Pending-step prose, ledgers, metrics logs, resume blocks, and workflow specs are not read by this path. Both formats fail rather than truncate above 8,192 bytes.
